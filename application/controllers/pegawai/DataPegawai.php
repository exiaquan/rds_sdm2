...
class DataPegawai extends CI_Controller {
	...
	public function index(){
		...
		if($this->session->userdata('sts_login') != true){
			...
			if($this->session->userdata('hak_akses') == 3){
				redirect('pegawai/datapegawai/update?id='.$this->session->userdata('hak_akses'));
			}else{
	...
	public function saveData(){
		...
		// Form validation
		$form->set_rules('id_pegawai','<b class="text-uppercase">ID</b>','required|is_unique[pegawai.id_pegawai]');
	...
	public function delete(){
		$id = $this->input->get('id');
		$this->crud->delData(array('id_pegawai'=>$id),'pegawai');
		redirect('pegawai/datapegawai');
	}
	...
	public function pdfGen(){
		...
		$data['pegawai'] = $this->crud->getDataWhere('pegawai',array('id_pegawai'=>$id))->row_array();
		$data['crud'] = $this->crud;
		$this->load->view('pegawai/mypdf',$data);
	...
	public function detail(){
		...
		if($this->session->userdata('sts_login') != true){
			...
			if($this->session->userdata('hak_akses') == 3){
				...
				$pegawai = $this->crud->getDataWhere('pegawai',array('id_pegawai'=>$id))->row_array();
				$data['title'] = 'Data Pegawai';
				$data['sub_title'] = $pegawai['id_pegawai'];
				$data['breadcrumb'] = '';
				$data['pegawai'] = $pegawai;
				$data['crud'] = $this->crud;
				$this->template->view('pegawai/detail',$data);
			}
		}
	}
